...
export class AddRecipeComponent implements OnInit {
  ...
  recipeForm: UntypedFormGroup = new UntypedFormGroup({
    ...
    flavourPalette: new UntypedFormControl(''),
    bestSeasonFrom: new UntypedFormControl(''),
    bestSeasonTo: new UntypedFormControl(''),
    description: new UntypedFormControl(''),
    preparationTimeValue: new UntypedFormControl(0),
    preparationTimeUnit: new UntypedFormControl('minute(s)'),
    cookingTimeValue: new UntypedFormControl(0),
    cookingTimeUnit: new UntypedFormControl('minute(s)'),
    servingTimeValue: new UntypedFormControl(0),
    servingTimeUnit: new UntypedFormControl('minute(s)'),
    shelfLifeValue: new UntypedFormControl(0),
    shelfLifeUnit: new UntypedFormControl('minute(s)'),
    flameThrow:new UntypedFormControl('',[]),
    finalPrice: new UntypedFormControl(''),
    energyConsumptionPerPreparation: new UntypedFormControl(''),
  });
  ...
  async submit() {
    ...
    if (this.recipeForm.errors==null) {
      ...
      this.servingInstructions.forEach((control: UntypedFormControl) => {
        servingInstructions.push(control.value);
        console.log('servingInstructions',control.value);
      });
      let cookingInstructions: string[] = [];
      this.cookingInstructions.forEach((control: UntypedFormControl) => {
      ...
      if(this.files){
        ...
        this.alertService.presentToast('Continuing without any photos.','error');
        // return
      }
  ...
  addInstruction(type: 'serving' | 'cooking') {
    if (type === 'serving') {
      this.servingInstructions.push(new UntypedFormControl('',[Validators.required]));
    } else {
      this.cookingInstructions.push(new UntypedFormControl('',[Validators.required]));
    }
  }
